...
// Account/instance label = sanitized basename of CLAUDE_CONFIG_DIR, or "default" (matches update.js).
...
function run() {
  ...
  if (event === "start") {
    ...
    try {
      ...
      writeAtomic(statePath, { state: "idle", label: "", tool: "", project: cwd ? path.basename(cwd) : "", cwd, instance: instanceLabel(), sessionId: id, transcript: "", entrypoint: process.env.CLAUDE_CODE_ENTRYPOINT || "", term_program: process.env.TERM_PROGRAM || "", pid: process.ppid, started: false, startedAt: 0, ts: Math.floor(Date.now() / 1000) });
    } catch {}
