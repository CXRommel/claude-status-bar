...
// Which account/instance is this — the sanitized basename of CLAUDE_CONFIG_DIR (e.g. "claude-work"),
// or "default" for a plain `claude`. Lets the app label a session by account when several are active.
...
process.stdin.on("end", () => {
  ...
  const out = { state, label, tool: p.tool_name || "", project, cwd, instance, sessionId: p.session_id || "", transcript: p.transcript_path || prev.transcript || "", entrypoint, term_program: termProgram, pid: process.ppid, started: true, startedAt, ts };
  try {
